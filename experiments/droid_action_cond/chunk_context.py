...
def render_chunk(
    *,
    ...
    # Chunk-local wrist anchor: GT wrist frame and FK pose at chunk-t=0.
    wrist0_img = wristf[0]
    T_hand0 = fk_urdf(joints[0])["hand"]
    T_wrist0_to_base = T_hand0 @ T_cam_to_hand
    ...
    # Wrist context has two modes (env WRIST_CONTEXT_MODE):
    ...
    wrist_mode = os.environ.get("WRIST_CONTEXT_MODE", "gripper_aware").lower()
    assert wrist_mode in ("simple", "gripper_aware"), \
        f"WRIST_CONTEXT_MODE must be 'simple' or 'gripper_aware', got {wrist_mode!r}"

    if wrist_mode == "gripper_aware":
        # One-shot gripper-only render at the chunk-t=0 wrist camera pose,
        # used as both an inpaint mask for the plate and a paste mask per frame.
        wrist_stencil_t0, _ = renderer.render(
            joints[0], K_wrist, T_wrist0_to_base,
            gripper_position=g0, gripper_only=True,
        )
        wrist0_gripper_mask = (wrist_stencil_t0.sum(-1) > 0)
        clean_bg_wrist0 = _clean_background_via_stencil(wrist0_img, wrist_stencil_t0)
        # Below this stroke threshold we paste the real wrist0 gripper pixels
        # (camera + gripper share a rigid frame -> pixel-identical when fingers
        # haven't moved). Above it, we render the mesh at the per-frame pose.
        gripper_eps = 0.03
    else:
        wrist_stencil_t0 = None
        wrist0_gripper_mask = None
        clean_bg_wrist0 = None
        gripper_eps = None
    ...
    for t in range(T):
        ...
        if wrist_mode == "simple":
            wrist_t = render_wrist_via_plane_homography(
                wrist0_img, K_wrist, T_wrist0_to_base,
                T_wrist_t_to_base, K_wrist, (W, H), plane_z=0.0,
            )
        else:  # gripper_aware
            bg_t = render_wrist_via_plane_homography(
                clean_bg_wrist0, K_wrist, T_wrist0_to_base,
                T_wrist_t_to_base, K_wrist, (W, H), plane_z=0.0,
            )
            if abs(g - g0) < gripper_eps:
                wrist_t = np.where(wrist0_gripper_mask[..., None], wrist0_img, bg_t)
            else:
                wrist_grip_t, _ = renderer.render(
                    joints[t], K_wrist, T_wrist_t_to_base,
                    gripper_position=g, gripper_only=True,
                )
                wrist_t = _composite_over_bg(wrist_grip_t, bg_t)
